...
class IRmarkCalculatorSpec extends BaseSpec {
  ...
  "IRmarkCalculator" - {
    ...
    "compute CIS IRmark " in {
      val body = Body(
        IRenvelope = IRenvelope(
          xmlns = "http://www.govtalk.gov.uk/taxation/CISreturn",
          IRheader = IRheader(
            Keys = List(
              Key(Type = "TaxOfficeNumber", Value = "754"),
              Key(Type = "TaxOfficeReference", Value = "EZ00047")
            ),
            ...
            IRmark = Some(IRmark(Type = "generic", Content = "nyDYUz9y7VNNAYSZbos3yJ29uAI=")),
            Sender = "Company"
          ),
          CISreturn = Some(
            CISreturn(
      ...
      println(irmark)
      irmark                                   should be("nyDYUz9y7VNNAYSZbos3yJ29uAI=")
      IRmarkCalculator.computeFullIRmark(body) should be("nyDYUz9y7VNNAYSZbos3yJ29uAI=")
    }
  }
}
