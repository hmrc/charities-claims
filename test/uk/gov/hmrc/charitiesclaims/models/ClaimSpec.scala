...
class ClaimSpec extends AnyWordSpec with Matchers {
  ...
  "Claim" should {
    "be serialised and deserialised correctly" in {
      ...
      claim.claimData.repaymentClaimDetails.claimReferenceNumber           shouldBe "New GA Claim"
      claim.claimData.organisationDetails.map(_.nameOfCharityRegulator)    shouldBe Some(
        "EnglandAndWales"
      )
      claim.claimData.organisationDetails.map(_.charityRegistrationNumber) shouldBe Some(
        "1137948"
      )
      claim.claimData.organisationDetails.map(_.areYouACorporateTrustee)   shouldBe Some(true)
      claim.claimData.organisationDetails.map(_.nameOfCorporateTrustee)    shouldBe Some(
        "Joe Bloggs"
      )
      claim.claimData.organisationDetails.map(_.corporateTrusteePostcode)  shouldBe Some(
        "AB12 3YZ"
      )
      claim.claimData.organisationDetails
        .map(_.corporateTrusteeDaytimeTelephoneNumber)                     shouldBe Some("071234567890")
