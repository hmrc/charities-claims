...
//        )
      ...
      when(
        rdsConnectorMock.getOrganisationName(ArgumentMatchers.eq(currentUser.enrolmentIdentifierValue))(using
          any[HeaderCarrier]
        )
      ).thenReturn(Future.successful(Some(orgName)))
    ...
    //      val service                       = new ChRISSubmissionServiceImpl(rdsConnectorMock, claimsValidationConnectorMock, appConfig)
    //
    //      when(rdsConnectorMock.getAgentName(ArgumentMatchers.eq("test-enrolment-identifier-value"))(using any())).thenReturn(Future.successful(Some("Test-Agent-Name")))
    //
    //      // (rdsConnectorMock.getAgentName(_: String)(using _: HeaderCarrier)).expects("test-enrolment-identifier-value", *).returns(Future.successful(Some("Test-Agent-Name")))
    //
    //      val claim = models.Claim(
    //        claimId = "test-claim-id",
    //        userId = "test-user-id",
